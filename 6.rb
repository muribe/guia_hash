...
genre = []
movies.each { |key, value| genre.push(value[:genre].downcase) }
genre.uniq!

new_hash_genre=Hash.new {|h,k| h[k]=[]}

genre.each do |value| 
    movies.each do |key, value2|  
       (value2[:genre].downcase==value) ? new_hash_genre[value] << value2: false  
    end
end
print "#{new_hash_genre}\n"
count = 0
genre.each do |value| 
    movies.each do |key, value2| 
       (value2[:genre].downcase==value) ? count+=1 : false  
    end
    puts "El genero #{value} tiene #{count} peliculas\n"
    count = 0 
end

movies_range= movies.select{ |key, value| value[:realease_date].to_s.split(//).last(4).join.to_i.between?(1999,2008)}

print "#{movies_range}\n"
bad=0
good=0
movies.each do |key, value|  
  value[:votes].each do |vote|
    if vote.to_s=="bad"
      bad+=1
    else
      good+=1    
  ...
  if bad>good
    puts "La pelicula #{key} ha sido evaluada como mala\n"
  elsif good>bad
    puts "La pelicula #{key} ha sido evaluada como buena\n"
  else
    puts "La pelicula #{key} ha sido evaluada como regular\n"
  end      
  bad=0
  good=0
end  
